...
class OnPolicyRunnerMulti(OnPolicyRunner):
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        num_catch_actor_obs = obs["catch_actor"].shape[2]
        ...
        actor_critic: ActorCriticMulti = ActorCriticMulti(
            num_prio_obs,
            num_catch_actor_obs,
            num_critic_obs,
        ...
        self.alg.init_storage(
            ...
            [history_length * (num_catch_actor_obs + num_prio_obs + num_image_obs)],
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        catch_actor_obs = obs["catch_actor"].flatten(start_dim=1).to(self.device)
        ...
        actor_obs = torch.cat([catch_actor_obs, prop_obs, image_obs], dim=1)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    prop_obs = (
                        obs["proprioception"].flatten(start_dim=1).to(self.device)
                    )  # proprioception
                    catch_actor_obs = (
                        obs["catch_actor"].flatten(start_dim=1).to(self.device)
                    )
                    # events_obs = obs["events"].flatten(start_dim=1)
                    image_obs = (
                        obs["image"]
